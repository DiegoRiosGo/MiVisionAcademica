...
def api_estadisticas_alumno(request):
    try:
        ...
        # --- Promedios por semestre (alumno y general) ---
        promedios_semestre = {}
        ...
        # --- Detección del modo de análisis ---
        modo_detallado = bool(filtro_anio or (filtro_area and filtro_area.lower() != "todas"))

        if modo_detallado:
            # 🔹 Filtrado activo → análisis por ASIGNATURA
            promedios_especifico = {}
            for d in datos_alumno:
                nombre_asig = d["asignatura"]["nombre_asignatura"] if d["asignatura"] else "Sin asignatura"
                promedios_especifico.setdefault(nombre_asig, []).append(float(d["calificacion"]))
            promedios_especifico = {k: round(sum(v)/len(v), 2) for k, v in promedios_especifico.items()}

            anio_asig = {}
            for d in datos_alumno:
                nombre_asig = d["asignatura"]["nombre_asignatura"] if d["asignatura"] else "Sin asignatura"
                clave = (d["acno"], nombre_asig)
                anio_asig.setdefault(clave, []).append(float(d["calificacion"]))
            promedios_especifico_anio = {
                f"{anio}-{nombre}": round(sum(v)/len(v), 2)
                for (anio, nombre), v in anio_asig.items()
            }

            tipo_datos = "asignatura"
        else:
            # 🔹 Sin filtros → análisis general por ÁREA
            promedios_especifico = {}
            for d in datos_alumno:
                area = d["asignatura"]["area"] if d["asignatura"] else "Sin área"
                promedios_especifico.setdefault(area, []).append(float(d["calificacion"]))
            promedios_especifico = {k: round(sum(v)/len(v), 2) for k, v in promedios_especifico.items()}
        ...
        return JsonResponse({
            "tipo_datos": tipo_datos,  # 🔹 indica si es “area” o “asignatura”
            "promedios_semestre": promedios_semestre,
            "promedios_especifico": promedios_especifico,
            "promedios_especifico_anio": promedios_especifico_anio,
...
def obtener_retroalimentaciones_alumno(request):
    try:
        ...
        for c in comentarios.data or []:
            ...
            # obtener nombre de la asignatura (usando asignatura_id correctamente)
            asig_res = supabase.table("asignatura").select("asignatura_id, nombre_asignatura, area").eq("asignatura_id", c["asignatura_id"]).maybe_single().execute()
            if asig_res.data:
                nombre_asignatura = asig_res.data.get("nombre_asignatura") or f"Asignatura ID {c['asignatura_id']}"
                area_asig = asig_res.data.get("area")
            else:
                nombre_asignatura = f"Asignatura ID {c['asignatura_id']}"
                area_asig = None
